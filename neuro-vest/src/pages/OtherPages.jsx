...
function RiscoPage() {
  const fatores = [
    { fator: 'Hipertensão Arterial', peso: 'Alto', contribuicao: 28, cor: 'var(--accent-orange)', status: 'danger' },
    { fator: 'Variabilidade FC reduzida', peso: 'Moderado', contribuicao: 18, cor: 'var(--accent-orange)', status: 'warning' },
    { fator: 'SpO₂ limítrofe', peso: 'Baixo', contribuicao: 8, cor: 'var(--accent-cyan)', status: 'normal' },
    { fator: 'Histórico familiar AVC', peso: 'Alto', contribuicao: 22, cor: 'var(--accent-red)', status: 'danger' },
    { fator: 'EEG dentro do normal', peso: 'Protetor', contribuicao: -12, cor: 'var(--accent-green)', status: 'normal' },
    { fator: 'Oxigenação cerebral ok', peso: 'Protetor', contribuicao: -8, cor: 'var(--accent-green)', status: 'normal' },
  ];
  ...
  const recomendacoes = [
    { prioridade: 'URGENTE', texto: 'Monitorar PA — sistólica persistentemente acima de 135 mmHg', icone: 'bi-exclamation-triangle-fill', cor: 'var(--accent-orange)' },
    { prioridade: 'IMPORTANTE', texto: 'Encaminhar para avaliação cardiológica em até 30 dias', icone: 'bi-calendar-check', cor: 'var(--accent-cyan)' },
    { prioridade: 'PREVENTIVO', texto: 'Considerar angiotomografia para rastreamento de aneurisma aórtico', icone: 'bi-shield-check', cor: 'var(--accent-green)' },
    { prioridade: 'ESTILO DE VIDA', texto: 'Redução de sódio e prática de atividade física moderada', icone: 'bi-heart-fill', cor: 'var(--accent-green)' },
  ];
  ...
  return (
    <div>
      <div className="row g-3 mb-4">
          ...
          <div className="card-dark h-100 text-center">
            ...
            <div style={{ position: 'relative', height: 180 }}>
              <ResponsiveContainer width="100%" height={180}>
                <RadialBarChart innerRadius="60%" outerRadius="90%" data={[{ value: 39 }]} startAngle={180} endAngle={0}>
                  <RadialBar dataKey="value" cornerRadius={10} fill="var(--accent-orange)" />
                </RadialBarChart>
              </ResponsiveContainer>
              <div style={{ position: 'absolute', top: '55%', left: '50%', transform: 'translate(-50%,-50%)', textAlign: 'center' }}>
                <div style={{ fontSize: 42, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--accent-orange)' }}>39</div>
        ...
        <div className="col-12 col-lg-4">
          <div className="card-dark h-100">
            ...
            <div className="d-flex justify-content-center gap-3 mt-2" style={{ fontSize: 11, fontFamily: 'var(--font-mono)' }}>
              {pieData.map(d => <span key={d.name}><span style={{ color: d.color }}>●</span> {d.name} {d.value}%</span>)}
            </div>
          </div>
        </div>
            ...
            <div className="d-flex flex-column gap-2">
              {fatores.map((f, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ flex: 1, fontSize: 12, color: 'var(--text-secondary)' }}>{f.fator}</div>
                  <div style={{ width: 60, textAlign: 'right', fontSize: 13, fontFamily: 'var(--font-mono)', color: f.cor, fontWeight: 700 }}>
      ...
      <div className="row g-3">
        {recomendacoes.map((r, i) => (
          <div className="col-12 col-md-6" key={i}>
            <div className="card-dark d-flex gap-3" style={{ borderLeft: `3px solid ${r.cor}` }}>
              <i className={`bi ${r.icone}`} style={{ color: r.cor, fontSize: 20, marginTop: 2 }}></i>
              <div>
                <div style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: r.cor, letterSpacing: 1, marginBottom: 4 }}>{r.prioridade}</div>
                <div style={{ fontSize: 13, color: 'var(--text-primary)' }}>{r.texto}</div>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="card-dark mt-4" style={{ background: '#001a0a', border: '1px solid #00e67622' }}>
        <div className="d-flex gap-2 align-items-start">
          <i className="bi bi-info-circle-fill" style={{ color: 'var(--accent-green)', fontSize: 16, marginTop: 2 }}></i>
          <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
            <strong style={{ color: 'var(--accent-green)' }}>Aviso Clínico:</strong> Este sistema não realiza diagnóstico médico. Os índices de risco são indicativos baseados em padrões fisiológicos e devem ser interpretados por profissional de saúde habilitado.
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      <div className="card-dark mb-4">
        <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
          ...
          <button className="btn-neuro"><i className="bi bi-download me-2"></i>Exportar CSV</button>
        </div>
        <div className="scroll-table">
          <table className="table table-dark-custom table-borderless mb-0">
            <thead>
              <tr>
                <th>Data/Hora</th><th>FC</th><th>PA Sis.</th><th>PA Dias.</th><th>SpO₂</th><th>Risco IA</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {fakeHistorico.map((row, i) => {
                ...
                return (
                  <tr key={i}>
                    <td style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)' }}>{row.data}</td>
                    <td><span style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent-green)' }}>{row.fc}</span> <span style={{ color: 'var(--text-muted)', fontSize: 11 }}>bpm</span></td>
                    <td><span style={{ fontFamily: 'var(--font-mono)', color: row.paSis > 135 ? 'var(--accent-orange)' : 'var(--text-primary)' }}>{row.paSis}</span></td>
                    <td><span style={{ fontFamily: 'var(--font-mono)', color: row.paDias > 89 ? 'var(--accent-orange)' : 'var(--text-primary)' }}>{row.paDias}</span></td>
                    <td><span style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent-blue)' }}>{row.spo2}%</span></td>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <div className="risk-bar" style={{ width: 60, height: 6 }}>
                          <div className="risk-fill" style={{ width: `${row.riscoGeral}%`, background: row.riscoGeral > 50 ? 'var(--accent-red)' : row.riscoGeral > 35 ? 'var(--accent-orange)' : 'var(--accent-green)' }} />
                        </div>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>{row.riscoGeral}%</span>
                      </div>
                    </td>
  ...
  return (
    <div>
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, rgba(141, 193, 223, 0.12), rgba(102, 69, 172, 0.10))',
        ...
        <div className="d-flex flex-column flex-lg-row justify-content-between gap-3 align-items-lg-center">
          <div>
            ...
            <div style={{ color: 'var(--text-secondary)', fontSize: 14 }}>
              Acompanhe os equipamentos conectados, autonomia, sinal e ações de manutenção em um painel visual.
            </div>
          </div>
          ...
          <div className="col-12 col-md-6" key={d.id}>
            <div className="card-dark h-100">
              ...
              <div className="row g-2 mb-3">
                <div className="col-6">
                  <div style={{ background: 'var(--bg-panel)', borderRadius: 8, padding: '10px 12px' }}>
                    <div className="metric-label">Bateria</div>
                    <div style={{ fontSize: 22, fontFamily: 'var(--font-mono)', color: d.bateria > 30 ? 'var(--accent-green)' : 'var(--accent-red)' }}>{d.bateria}%</div>
                    <div className="risk-bar mt-1"><div className="risk-fill" style={{ width: `${d.bateria}%`, background: 'var(--accent-green)' }} /></div>
                  </div>
                </div>
                <div className="col-6">
                  <div style={{ background: 'var(--bg-panel)', borderRadius: 8, padding: '10px 12px' }}>
                    <div className="metric-label">Sinal</div>
                    <div style={{ fontSize: 22, fontFamily: 'var(--font-mono)', color: 'var(--accent-blue)' }}>{d.sinal}%</div>
                    <div className="risk-bar mt-1"><div className="risk-fill" style={{ width: `${d.sinal}%`, background: 'var(--accent-blue)' }} /></div>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-3">
                ...
                <div className="d-flex flex-wrap gap-2">
                  {d.sensores.map(s => <span key={s} className="tag tag-blue">{s}</span>)}
                </div>
              </div>

              <div className="d-flex gap-2 flex-wrap">
                <button className="btn-neuro" onClick={() => handleComando(d.id, 'calibrar')}>
                  <i className="bi bi-sliders me-1"></i>Calibrar
                </button>
                <button className="btn-neuro" onClick={() => handleComando(d.id, 'reiniciar')}>
                  <i className="bi bi-arrow-clockwise me-1"></i>Reiniciar
                </button>
                <button className="btn-neuro" style={{ borderColor: 'var(--accent-red)', color: 'var(--accent-red)' }} onClick={() => handleComando(d.id, 'desligar')}>
      ...
      <div className="row g-4">
        <div className="col-12 col-lg-5">
          <div className="card-dark">
            ...
            <div className="d-flex flex-column gap-3">
              {[
                ['ID', patient.id], ['Nome', patient.nome], ['Idade', `${patient.idade} anos`],
                ['Sexo', patient.sexo], ['Peso', `${patient.peso} kg`], ['Altura', `${patient.altura} cm`],
                ['IMC', `${(patient.peso / (patient.altura / 100) ** 2).toFixed(1)}`],
                ['Médico Resp.', patient.medico],
              ].map(([k, v]) => (
                <div key={k} className="d-flex justify-content-between" style={{ borderBottom: '1px solid var(--border)', paddingBottom: 10 }}>
        ...
        <div className="col-12 col-lg-7">
          <div className="card-dark mb-4">
            ...
            <div className="d-flex flex-wrap gap-2">
              {patient.condicoes.map(c => <span key={c} className="tag tag-orange" style={{ fontSize: 13, padding: '6px 12px' }}>{c}</span>)}
            </div>
          </div>
          <div className="card-dark">
            <div className="section-header">Última Sessão</div>
            <div style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent-cyan)', fontSize: 18 }}>
              {new Date(patient.ultimaSessao).toLocaleString('pt-BR')}
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8 }}>
              Próxima sessão recomendada: <span style={{ color: 'var(--text-primary)' }}>Em 24 horas</span>
            </div>
          </div>
        </div>
      </div>
...
function getRespostaProfissional(profissao, texto) {
  ...
  if (profissao === 'Psicólogo(a)') {
    ...
    if (mensagem.includes('triste') || mensagem.includes('sozinho') || mensagem.includes('não estou bem') || mensagem.includes('n estou bem')) {
  ...
  if (profissao === 'Médico(a)') {
    if (mensagem.includes('dor') || mensagem.includes('falta de ar') || mensagem.includes('tontura')) {
...
function ContatosPage({ patient }) {
  const profissionais = [
    {
      ...
      abertura: 'Olá! Sou sua psicóloga de apoio. Pode me contar como você está hoje.',
    },
    {
      key: 'medico',
      cargo: 'Médico(a)',
      ...
      abertura: 'Olá, estou acompanhando seus dados clínicos. Quais sintomas você deseja relatar agora?',
    },
    {
      key: 'fisio',
      ...
      status: 'Online',
      cor: 'var(--accent-blue)',
      abertura: 'Vamos cuidar da sua mobilidade. Me diga como está dor, cansaço ou respiração hoje.',
    },
  ];
  ...
  const [conversas, setConversas] = useState(() => {
    ...
    profissionais.forEach((p) => {
      base[p.key] = [
        { id: `${p.key}-1`, sender: 'pro', text: p.abertura },
      ];
    });
    return base;
  });
        ...
        <div className="d-flex flex-column flex-md-row gap-3 justify-content-between align-items-md-center">
          <div>
            ...
            <div style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
              Converse diretamente com psicólogo, médico e fisioterapeuta em um só lugar.
            </div>
          </div>
        ...
        <div className="col-12 col-lg-8">
          <div className="card-dark h-100">
            <div className="section-header">
              Chat com {profissionalAtivo?.cargo}
            </div>
...
function getAssistantReply(message) {
  ...
  if (texto.includes('ansioso') || texto.includes('ansiedade') || texto.includes('nervoso')) {
  ...
  if (texto.includes('suic') || texto.includes('me machucar') || texto.includes('morrer')) {
...
function IAsuportePage() {
  ...
  const atalhos = useMemo(
    () => ['n estou muito bem', 'estou ansioso', 'me sinto sozinho', 'preciso de ajuda'],
    [],
  );
      ...
      <div className="card-dark mb-4">
        <div className="d-flex flex-column flex-md-row gap-3 justify-content-between align-items-md-center">
          <div>
            ...
            <div style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
              Conversa de apoio emocional inicial e orientação para próximos passos.
            </div>
          </div>
        ...
        <div className="col-12 col-lg-8">
          <div className="card-dark h-100">
            ...
            <div className="support-quick-actions mt-3">
              {atalhos.map((atalho) => (
                <button key={atalho} type="button" className="support-quick-btn" onClick={() => enviarMensagem(atalho)}>
                  {atalho}
                </button>
              ))}
            </div>
        ...
        <div className="col-12 col-lg-4">
          <div className="card-dark">
            ...
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
              Esta IA oferece apoio inicial e não realiza diagnóstico. Em caso de dor intensa,
              falta de ar, confusão, risco de autoagressão ou qualquer urgência, procure atendimento
              humano imediato.
            </div>
          </div>
        </div>
